    ...
    def plot_tx(self, pred: bool = False, digitized_liquidus: list = None, gas_temp: int | float = None, ternary_color_map: list = None) -> go.Figure:
        ...
        fig.update_layout(
            title=dict(text=f'<b>{self.comps[0]}-{self.comps[1]} DFT-Referenced Phase Diagram</b>',
                   x=0.5, xanchor='center', font=dict(size=15, color='black'), yanchor='bottom'),
            ...
            margin=dict(t=30, b=0, r=0, l=25)  # Also reduce top margin for tighter spacing
        )
        axes_params_dict = dict(
            title_font=dict(size=13),
            title_standoff=12,  # Space between title and axis line
        ...
        fig.add_annotation(
            x=-0.03,
            y=-0.15,  # Position below the x-axis in paper coordinates
            xref="paper",
            yref="paper",
